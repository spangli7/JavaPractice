...
ellenfel_gol INT,
fel_hazai_gol INT,
fel_ellenfel_gol INT,
csapat_1 VARCHAR(255),
csapat_2 VARCHAR(255)
);

INSERT INTO data (fordulo, hazai_gol, ellenfel_gol, fel_hazai_gol, fel_ellenfel_gol, csapat_1, csapat_2)
VALUES
...
CREATE TABLE foci_team
(id INT AUTO_INCREMENT PRIMARY KEY,
name VARCHAR(255));

INSERT INTO foci_team (name)
SELECT DISTINCT name FROM (
SELECT csapat_1 as name FROM data
UNION
SELECT csapat_2 as name FROM data) a;

CREATE TABLE foci_match
(
 id INT AUTO_INCREMENT PRIMARY KEY,
 home_team_id INT,
 opp_team_id INT,
 ...
 half_time_opp_goals INT
);

INSERT INTO foci_match (home_team_id, opp_team_id, round, home_goals, opp_goals, half_time_home_goals, half_time_opp_goals)
SELECT FCT1.id, FCT2.id, D.fordulo, D.hazai_gol, D.ellenfel_gol, D.fel_hazai_gol, D.fel_ellenfel_gol
FROM data D
INNER JOIN foci_team FCT1 ON FCT1.name = D.csapat_1
INNER JOIN foci_team FCT2 ON FCT2.name = D.csapat_2;
...
SELECT *
FROM foci_match FM
INNER JOIN foci_team FCT1 ON FCT1.id = FM.home_team_id
INNER JOIN foci_team FCT2 ON FCT2.id = FM.opp_team_id
ORDER BY FM.round;